...
@section('title', 'Holidays - Admin Dashboard')
...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-center">
        <div>
            <p class="text-sm text-gray-500">Manage holidays and flag placement schedules</p>
        </div>
    ...
    <div class="bg-white shadow rounded-lg p-6">
        <form method="GET" action="{{ route('admin.holidays.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div class="flex items-end space-x-2">
                ...
                <a href="{{ route('admin.holidays.index') }}" class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                    Clear
                </a>
            </div>
        </form>
                ...
                @forelse($holidays as $holiday)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            @if(isset($holiday->recurring) && $holiday->recurring)
                                <div class="text-xs text-gray-500">Recurring annually</div>
                            @endif
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900">
                                <span class="text-gray-500">Place:</span> {{ $holiday->placement_days_before ?? 1 }} days before
                            </div>
                            <div class="text-sm text-gray-500">
                                <span>Remove:</span> {{ $holiday->removal_days_after ?? 1 }} days after
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $holiday->active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                                {{ $holiday->active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                            {{ $holiday->sort_order ?? 0 }}
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center text-sm text-gray-500">
                            ...
                            <a href="{{ route('admin.holidays.create') }}" class="mt-2 inline-flex items-center text-indigo-600 hover:text-indigo-900">
