...
<div class="space-y-6" x-data="flagProducts()">
    ...
    <div x-show="showInventoryModal" x-cloak class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            <div x-show="showInventoryModal" x-transition:enter="ease-out duration-300" 
                    ...
                    <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                <h3 class="text-lg leading-6 font-medium text-gray-900">Adjust Inventory</h3>
                                <div class="mt-4 space-y-4">
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Adjustment</label>
                                        <input type="number" x-model="inventoryForm.adjustment" 
                                        ...
                                        <p class="mt-1 text-xs text-gray-500">Positive numbers add inventory, negative numbers remove inventory</p>
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Reason</label>
                                        <select x-model="inventoryForm.reason" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                            <option value="restock">Restock</option>
                                            <option value="damage">Damaged inventory</option>
                                            <option value="loss">Lost inventory</option>
                                            <option value="correction">Inventory correction</option>
                                            <option value="other">Other</option>
                                        </select>
                                    </div>
                                    <div x-show="inventoryForm.reason === 'other'">
                                        <label class="block text-sm font-medium text-gray-700">Custom Reason</label>
                                        <input type="text" x-model="inventoryForm.customReason" 
                                               class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                    </div>
                                </div>
                            </div>
...
function flagProducts() {
    return {
        ...
        submitInventoryAdjustment() {
            const reason = this.inventoryForm.reason === 'other' 
                ? this.inventoryForm.customReason 
                : this.inventoryForm.reason;
            ...
            fetch(`/admin/flag-products/${this.inventoryProductId}/adjust-inventory`, {
                ...
                body: JSON.stringify({
                    adjustment: parseInt(this.inventoryForm.adjustment),
                    reason: reason
                })
            })
            .then(response => response.json())
